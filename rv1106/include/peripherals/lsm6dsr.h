...
private:
    bool _debug, _fifo_mode;
    explicit Lsm6dsr(int bus, uint8_t addr, bool debug=false, bool fifo_mode=false);
public:
    ~Lsm6dsr() noexcept;

    static std::unique_ptr<Lsm6dsr> create(int bus, uint8_t addr,
    ...
    static std::unique_ptr<Lsm6dsr> create(int bus, uint8_t addr,
                                           uint8_t AccFlag, uint8_t GyroFlag,
                                           uint8_t FifoRate,
                                           bool debug=false, bool fifo_mode=true);

    bool init(uint8_t AccFlag, uint8_t GyroFlag);
    ...
    bool readImuData(ImuData &imu_data, uint8_t &is_new_data);
    bool readImuData(ImuData &imu_data);
