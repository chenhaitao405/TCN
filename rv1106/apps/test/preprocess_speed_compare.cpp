...
inline int NCHW_float32_to_NC1HWC2_int8(const float *src, int8_t *dst, int batch, int channel, int h, int w, float scale, int zp,
...
int main(int argc, char **argv) {
    ...
    // 初始化 src (NHWC 布局: src[w*8+c])
    srand(42);
    for (int i = 0; i < W * 8; ++i) {
        src[i] = (float)(rand() % 2000 - 1000) / 10.0f; // -100.0 ~ 100.0
    }
    // 为 NCHW 函数创建转置副本: src_nchw[c*W+w] = src[w*8+c]
    float *src_nchw = (float*)aligned_alloc(16, W*8*sizeof(float));
    for (int c = 0; c < 8; ++c) {
        for (int ww = 0; ww < W; ++ww) {
            src_nchw[c * W + ww] = src[ww * 8 + c];
        }
    }
    ...
    int8_t *dst3 = (int8_t*) aligned_alloc(16, W*ALIGNED_CHANNEL);
    memset(dst3, 0, W*ALIGNED_CHANNEL);
    // warmup
    for (int i=0; i<3; ++i) {
    ...
    for (int i=0; i<3; ++i) {
        NCHW_float32_to_NC1HWC2_int8(src_nchw, dst3, 1, 8, 1, W, scale, zp, mean, std, 8);
    }
    ...
    auto during_time3 = std::chrono::duration_cast<std::chrono::microseconds>(end_time3 - start_time3).count();
    std::cout << during_time3 << "us" << std::endl;

    for (int i=0; i<W*ALIGNED_CHANNEL; ++i) {
        if (dst2[i] != dst[i]) {
            std::cout << "a result wrong\n";
            free(src);
            free(src_nchw);
            free(dst);
            free(dst2);
    ...
    for (int i=0; i<W*ALIGNED_CHANNEL; ++i) {
        if ((int32_t)dst3[i] - (int32_t)dst[i] > 1 || (int32_t)dst3[i] - (int32_t)dst[i] < -1) {
            std::cout << "b result wrong\n";
            std::cout << (int32_t)dst3[i] << ' ' << (int32_t)dst[i] << std::endl;
            free(src);
            free(src_nchw);
            free(dst);
            free(dst2);
