...
void send_1(std::unique_ptr<SerialBase> sp) {
    while (g_running) {
        sp->send("+1\r\n");
...
int main(int argc, char **argv) {
    ...
    std::unique_ptr<SerialBase> uart_port = SerialBase::create(argv[1], B115200);
    GpioPin gpio(atoi(argv[2]), GpioPin::DIRECTION::OUT);

    std::thread uart_thread(send_1, std::move(uart_port));
