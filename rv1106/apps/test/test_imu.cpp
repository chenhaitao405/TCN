        ...
        if (!data.empty()) {
            ...
            if (sample_count % 20 < data.size()) {
                ...
                if (sensor->is_debug()) std::cout << "[Debug] Samples: " << sample_count 
                          << " | Rate: " << rate << " Hz"
                          << " | XL: " << last_data.acc_x << ", " << last_data.acc_y << ", " << last_data.acc_z
                          << " | GY: " << last_data.gyro_x << ", " << last_data.gyro_y << ", " << last_data.gyro_z
                          << std::endl;
            }
        }
...
void data_acquisition_task(Lsm6dsr *sensor, GpioPin *gpio) {
    ...
    while (running) {
        ...
        int ret = epoll_wait(epfd, &events[0], 1, 20);
        if (ret > 0) {
                ...
                if (sensor->is_debug() && interrupt_count % 20 == 0){
                    if (!data.empty()) {
                        ...
                        for (const auto& d : data) {
                                std::cout << " | ACC: " << d.acc_x << ", " << d.acc_y << ", " << d.acc_z
                                << " | GYRO: " << d.gyro_x << ", " << d.gyro_y << ", " << d.gyro_z << std::endl;
                            }
                        }
                    else {
...
int main(int argc, char* argv[]) {
    ...
    if (use_polling) {
        ...
        std::thread acquisition_thread(data_acquisition_task_polling, lsm.get());
        ...
        std::thread acquisition_thread(data_acquisition_task, lsm.get(), &int1);
