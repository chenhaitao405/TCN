...
void print_usage(const char *prog_name)
{
    ...
    printf("Description:\n");
    printf("    Run knee action classification model inference.\n\n");
    ...
    printf("    -t, --transfer      Enable pattern transfer (encoded in time_val)\n");
    printf("    -d, --debug         Debug to show more information\n");
    printf("    -h, --help          Show this help message and exit\n\n");
    printf("Examples:\n");
    printf("    %s -t -s l model.rknn /dev/ttyUSB0\n\n", prog_name);
}
...
int main(int argc, char **argv)
{
    ...
    while (g_running)
    {
        ...
        ret = fast_NHWC_float32_circular_buffer_to_NC1HWC2_int8<Knee::DataFrame>(src_buffer,
        ...
        uint8_t pattern = get_pattern_from_output<Knee::NumClasses>(out_fp16, logits);

        ++send_count;
        if (transfer_pattern && send_count % 2 == 0)
            knee_producer::sendMoment(0.0f, static_cast<float>(pattern));

        if (debug && infer_count % 50 == 0)
        {
            double avg_infer_ms = total_infer_ms / infer_count;
            printf("[%5d] pattern=%u | logits=[%.3f, %.3f, %.3f, %.3f] | avg=%.2fms | %.1f fps\n",
                   infer_count,
                   pattern,
