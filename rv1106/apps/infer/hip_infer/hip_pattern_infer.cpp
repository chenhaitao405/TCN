...
void print_usage(const char *prog_name)
{
    ...
    printf("Description:\n");
    printf("    Run hip action classification model inference.\n\n");
    ...
    printf("    -t, --transfer      Enable pattern transfer to exoskeleton\n");
    printf("    -d, --debug         Debug to show more information\n");
    printf("    -h, --help          Show this help message and exit\n\n");
    printf("Examples:\n");
    printf("    # Run inference on left leg with pattern transfer\n");
    printf("    %s -t -s l model.rknn /dev/ttyS3\n\n", prog_name);
}
...
int main(int argc, char **argv)
{
    static const struct option long_options[] = {
        ...
        {"transfer", no_argument, nullptr, 't'},
    ...
    // 解析选项
    char side = 'l';
    bool transfer_pattern = false;
    bool debug = false;
    int opt;

    while ((opt = getopt_long(argc, argv, "s:tdh", long_options, nullptr)) != -1)
    {
        switch (opt)
        {
        case 's':
        ...
        case 't':
            printf("pattern transfer function is on\n");
    ...
    printf("Leg side: %s\n", side == 'l' ? "LEFT" : "RIGHT");
    printf("Inference: %s\n", "enabled");
    printf("Pattern transfer: %s\n", transfer_pattern ? "enabled" : "disabled");
    ...
    std::vector<rknn_tensor_mem *> input_mems, output_mems;
    ...
    while (g_running)
    {
        ...
        ret = fast_NHWC_float32_circular_buffer_to_NC1HWC2_int8<Hip::DataFrame>(src_buffer, (int8_t *)input_mems[0]->virt_addr, Hip::STREAM_LENGTH,
                                                                input_attrs[0].scale, input_attrs[0].zp, Hip::g_mean_vals, Hip::g_std_vals, Hip::ModelChannel);
        if (ret < 0)
        {
        ...
        uint8_t pattern = get_pattern_from_output<Hip::NumClasses>(out_fp16, logits);

        // 发送pattern指令 (每2次推理下发一次，保持100Hz)
        ++send_count;
        if (transfer_pattern && send_count % 2 == 0)
            hip_producer::sendData(0.0f, pattern, 0x00);

        if (debug && infer_count % 50 == 0)
        {
            double avg_infer_ms = total_infer_ms / infer_count;
            printf("[%5d] pattern=%d (%s) | logits=[%.3f, %.3f, %.3f, %.3f] | avg=%.2fms | %.1f fps\n",
                   infer_count, pattern, Hip::ACTION_NAMES[pattern],
