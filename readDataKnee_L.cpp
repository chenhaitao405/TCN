...
public:
    ...
    bool update(const KneeData& data) {
        ...
        const float acc_norm = norm3(data.acc);
        const float gyro_norm = norm3(data.gyro);

        // 采集初始站立姿态
        if (!initial_set_ && acc_norm > 0.5f * g_value_) {
            copy3(data.acc, initial_orientation_);
        ...
        case Stage::Still: {
            ...
            if (detectStillness(acc_norm, gyro_norm)) {
                if (still_dt >= min_still_duration_) {
                    if (orientationChanged(data.acc)) {
                        fall_detected_ = true;
                        reset();
                        return true;
                    } else {
...
bool start_imu_driver(const std::string& portName) {
    ...
    g_driver->setCallback([&](const KneeData& data) {
...

        {
            ...
            // 跌倒检测判断
            const bool fall_event = g_fall_detector.update(data_frame);
            if (fall_event) {
                std::cout << "[FallDetector] Potential fall detected" << std::endl;
            }
