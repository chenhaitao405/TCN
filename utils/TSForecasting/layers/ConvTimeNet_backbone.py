...
from utils.TSForecasting.layers.Patch_layers import *
from utils.TSForecasting.layers.RevIN import RevIN
...
class ConvTimeNet_backbone(nn.Module):
	def __init__(self, c_in:int, seq_len:int, context_window:int, target_window:int, patch_len:int, stride:int, 
				 ...
				 deformable=True, enable_res_param=True, re_param=True, re_param_kernel=3,final_out=2,
				 # 新增参数：降维后的通道数
				 reduced_channels=8,
				 center: float = 0.,
				 scale: float = 1.,
		...
		# 添加通道降维层（在Flatten之前）
		# 从 d_model (32) 降维到 reduced_channels (8)
		self.channel_reduction = nn.Conv1d(d_model, reduced_channels, kernel_size=1, stride=1, padding=0)
		
		# Head - 使用降维后的特征维度
		self.head_nf = reduced_channels * patch_num  # 8 * 187 = 1496
		self.n_vars = c_in
		self.head_type = head_type

		if head_type == 'flatten': 
			self.head = Flatten_Head(self.n_vars, self.head_nf, target_window, 
									head_dropout=head_dropout, output_channels=1) 
		else:
			raise ValueError(f'No such head@ {self.head}')
		
		# 最终投影层：从 [bs, n_vars, target_window] 到 [bs, target_window, 1]
		self.final_projection = nn.Sequential(
			nn.Linear(c_in, final_out),  # 将n_vars(5)维度降到1
	...
	def forward(self, z):  # z: [bs x nvars x seq_len]
		...
		# 通道降维：在head之前进行
		# z shape: [bs, nvars, d_model, patch_num] -> [bs, nvars, reduced_channels, patch_num]
		bs, nvars, d_model, patch_num = z.shape
		
		# 方法：先转置，应用Conv1d，再转置回来
		# [bs, nvars, d_model, patch_num] -> [bs*nvars, d_model, patch_num]
		z = z.reshape(bs * nvars, d_model, patch_num)
		# 应用1x1卷积降维
		z = self.channel_reduction(z)  # [bs*nvars, reduced_channels, patch_num]
		reduced_channels = z.shape[1]
		# 恢复维度 [bs*nvars, reduced_channels, patch_num] -> [bs, nvars, reduced_channels, patch_num]
		z = z.reshape(bs, nvars, reduced_channels, patch_num)
		...
		# 最终降维：[bs, n_vars, target_window] -> [bs, target_window, 1]
		z = z.permute(0, 2, 1)  # [bs, target_window, n_vars]
		z = self.final_projection(z)  # [bs, target_window, 1]
		z = z.permute(0, 2, 1)  # [bs, target_window, n_vars]
...
class Flatten_Head(nn.Module):
    def __init__(self, n_vars, nf, target_window, head_dropout=0, output_channels=1):
        super().__init__()
