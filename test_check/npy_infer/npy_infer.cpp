...
static const float g_mean_vals[8] = {0.7113f, -0.4300f, 0.7254f, 2.6529f, 8.7143f, -0.2818f, -30.6663f, -0.1348f};
static const float g_std_vals[8]  = {21.3148f, 45.2281f, 81.4140f, 3.9808f, 4.4334f, 1.8336f, 27.8284f, 107.1712f};
...
void send_int8_list_adb(const int8_t* data, size_t size, size_t stride) {
    FILE* tmp = fopen("/tmp/adb_data.bin", "wb");
    if (tmp) {
        for (size_t i=0; i<size; i+=stride) {
            fwrite(&data[i], 1, 1, tmp);
        }
        fclose(tmp);
        system("scp /tmp/adb_data.bin sxs@169.254.173.20:/home/sxs/TCN/rknn_related");
    }
    printf("output have sent to /home/sxs/TCN/rknn_related\n");
}
