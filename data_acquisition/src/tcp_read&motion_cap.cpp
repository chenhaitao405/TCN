...
// 定义 Socket 描述符类型，保持与原代码变量名兼容，但在 Linux 下是 int
typedef int SOCKET;
...
{
    // 注意：这里的路径是 Windows 风格，因为这通常是发送给运行在 Windows 上的动捕软件的指令
    // 如果接收端是 Linux，请修改 XML 中的 DatabasePath
    ...
    close(sock); // 使用 close 代替 closesocket
...
void processIMU()
{
    while (!flg_exit) 
    {
        ...
        // Linux 下 recv 与 Windows 类似，但 flags 通常为 0
        int bytes_read = recv(new_socket, buffer, BUFFER_SIZE, 0);

        if (bytes_read < 0) {
            std::cerr << "recv failed or error: " << strerror(errno) << std::endl;
            break; // 链接出错
        }
        ...
        while (buff_dy.size() >= 36) { // 24 = 2 (head) + 20 (data) + 2 (tail)
            ...
            // 注意：此处省略了 IMU7 到 IMU13 的逻辑，如果需要请保持原样复制粘贴
            // 逻辑与上方 IMU1-6 完全一致，仅需修改 HEAD/END 宏和对应的 imuEulerStreamX
            ...
            else
            {
                ...
                // 简单的检查避免过于频繁打印
                if (temp_buffer[0] == IMU1_HEAD1 || temp_buffer[0] == IMU2_HEAD1 || /*...*/ temp_buffer[0] == IMU13_HEAD1) {
                    known_head = true; 
                }
                
                if(!known_head) {
                     // std::cout << "all imu head wrong... " << std::endl;
                }
                find_new_head = true;
            }
        }
...
int main(int argc, char ** argv) {
    ...
    // 创建套接字
    if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) { // Linux 检查 < 0
        perror("socket failed");
        exit(EXIT_FAILURE);
    }

    // 设置套接字选项
    // Linux setsockopt 不需要 (const char*) 转换
