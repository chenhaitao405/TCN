...
class BoxCoder(nn.Module):
	def __init__(self, patch_count, patch_stride, patch_size, seq_len, channels, device='cuda:0' if torch.cuda.is_available() else 'cpu'):
		super().__init__()
		self.device = device
	...
	def _generate_anchor(self, device='cuda:0' if torch.cuda.is_available() else 'cpu'):
		anchors = []
